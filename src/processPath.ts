...
export const processPath = async (
  ...
  switch (true) {
    ...
    case reqUrlString.startsWith(USERS_PATH_SLASH) && req.method === METHOD_GET:
    ...
    case reqUrlString.startsWith(USERS_PATH_SLASH) && req.method === METHOD_PUT:
      const userIdPut = getPathParamId(reqUrlString);
      updateUser(req, res, userIdPut);
      break;

    case reqUrlString.startsWith(USERS_PATH_SLASH) && req.method === METHOD_DELETE:
